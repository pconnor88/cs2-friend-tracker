...
interface FriendlyFireLeaderboardSectionProps {
    period: StatPeriod;
    mapName?: string;
}
...
const ROWS: LeaderboardRowDef[] = [
    ...
    { label: "Friendly nade dmg / round", extract: s => s.friendlyHeDamagePerRound + s.friendlyMolotovDamagePerRound, higherIsBetter: false, format: { kind: "number", decimals: 2 } }
];
...
export const FriendlyFireLeaderboardSection = ({ period, mapName }: FriendlyFireLeaderboardSectionProps) => {
    const { data, isLoading } = useStatsForAllPlayers(period, undefined, mapName);

    if (isLoading || data === undefined) {
        return (
            <PageSection title="Friendly fire leaderboard" action={legend}>
